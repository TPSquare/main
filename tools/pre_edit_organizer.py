...
class PreEditOrganizer:
    ...
    def create_widgets(self):
        ...
        tk.Button(frame_controls, text="⬆ Lên", command=self.move_up, width=12).pack(
            side=tk.LEFT, padx=3
        )
        tk.Button(
        ...
        prefix_frame = tk.Frame(bottom_frame)
        prefix_frame.pack(fill=tk.X, pady=(0, 10))

        tk.Label(
            prefix_frame,
            text="Tiền tố tên file (để trống nếu chỉ muốn số):",
            font=("Arial", 10),
        ).pack(side=tk.LEFT)

        self.entry_prefix = tk.Entry(prefix_frame, font=("Arial", 11))
        self.entry_prefix.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(10, 0))
        ...
        tk.Button(
            buttons_frame,
            text="Chỉ đổi Date Modified",
            command=self.update_date_only,
            bg="#2e7d32",
            fg="white",
            font=("Arial", 12, "bold"),
            height=2,
        ...
        try:
            if ext in {".mp4", ".mov", ".avi", ".mkv", ".mp3", ".wav", ".aac", ".m4a"}:
                ...
                if ext in {".mp3", ".wav", ".aac", ".m4a"}:
                    self.lbl_img_preview.pack(expand=True, fill=tk.BOTH)
                    self.lbl_img_preview.config(
                        image="", text=f"🎵 Đang phát âm thanh:\n{filename}"
                    )
                else:
    ...
    def update_date_only(self):
        ...
        if total_items == 0:
            messagebox.showinfo(
                "Thông báo", "Không có file nào trong danh sách ĐÃ SẮP XẾP để đổi Date!"
            )
            return
        ...
        try:
            ...
            for i, filename in enumerate(all_items):
                ...
                try:
                    ...
                    print(f"Không thể cập nhật Date cho {filename}: {date_err}")

                self.progress_bar["value"] += 1
                self.lbl_progress.config(
                    text=f"Đang cập nhật Date cho file {filename}... ({i+1}/{total_items})"
                )
            ...
            messagebox.showinfo(
                "Thành công",
                f"Đã cập nhật Date Modified cho {len(all_items)} file theo thứ tự sắp xếp!",
            )
    ...
    def rename_files_in_place(self):
        ...
        if total_items == 0:
            messagebox.showinfo(
                "Thông báo", "Không có file nào trong danh sách ĐÃ SẮP XẾP để đổi tên!"
            )
            return
        ...
        try:
            for f in os.listdir(self.input_dir):
                ...
                if prefix:
                    if name_without_ext.startswith(prefix + " "):
                        num_part = name_without_ext[len(prefix) + 1 :]
                        if num_part.isdigit():
            ...
            print(f"Lỗi khi quét chỉ số cũ: {e}")
        ...
        try:
            ...
            for i, filename in enumerate(all_items):
                ...
                if prefix:
                    final_name = f"{prefix} {current_idx:03d}{ext}"
                else:
                    final_name = f"{current_idx:03d}{ext}"

                temp_name = f"__temp_preedit_{current_idx:03d}{ext}"
                temp_path = os.path.join(self.input_dir, temp_name)

                os.rename(old_path, temp_path)
                temp_renamed_files.append((temp_path, final_name))
            ...
            for i, (temp_path, final_name) in enumerate(temp_renamed_files):
                ...
                try:
                    ...
                    print(f"Không thể cập nhật Date cho {final_name}: {date_err}")
            ...
            messagebox.showinfo(
                "Thành công",
                f"Đã đổi tên và cập nhật ngày cho {len(all_items)} file!\nChỉ số bắt đầu từ: {start_idx:03d}",
            )
